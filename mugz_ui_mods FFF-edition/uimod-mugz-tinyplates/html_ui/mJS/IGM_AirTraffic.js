...
class IGM_TrafficElement_3D extends InGameMarkerBaseElement {
    ...
    setData(data) {
		...
		//# new
		var textOfInnerHTML = this.m_name.innerHTML;
		if (textOfInnerHTML.includes("FootanMouth")) {
			this.m_name.innerHTML = 'Footy';
		    this.m_name.classList.add('footy');
			this.m_dataElem.classList.add('footy');
		} else {
			this.m_name.classList.add('other');
			this.m_dataElem.classList.add('other');
		}
		if (textOfInnerHTML.includes("#")) {
			this.m_name.innerHTML = textOfInnerHTML.substring(0, textOfInnerHTML.indexOf("#"));
		}
		//# endnew
